...
grep -q '^ENABLE_A2DP_OFFLOAD_DISABLE=0
...
# Out-of-range values must be ignored rather than destabilising the watchdog.
...
# Structured telemetry must remain bounded and valid enough for line-oriented consumers.
...
echo "Bluetooth Stability Helper tests: PASS"
 "$MODDIR/common/config.sh"
grep -q '^METRICS_HISTORY_MAX_LINES=500
grep -q '^APPLY_APP_OPS_FIXES=0$' "$MODDIR/common/config.sh"
! grep -q '\[ -f "$USERCFG" \] && \. "$USERCFG"' "$MODDIR/service.sh"
! grep -q '\[ -f "$LOCAL_USER_CONFIG" \] && \. "$LOCAL_USER_CONFIG"' "$MODDIR/scripts/diagnostics.sh"
grep -q '^SKIPUNZIP=0$' "$MODDIR/customize.sh"
...
echo "Bluetooth Stability Helper tests: PASS"
 "$MODDIR/common/config.sh"
grep -q '^EVENT_HISTORY_MAX_LINES=500
grep -q '^APPLY_APP_OPS_FIXES=0$' "$MODDIR/common/config.sh"
! grep -q '\[ -f "$USERCFG" \] && \. "$USERCFG"' "$MODDIR/service.sh"
! grep -q '\[ -f "$LOCAL_USER_CONFIG" \] && \. "$LOCAL_USER_CONFIG"' "$MODDIR/scripts/diagnostics.sh"
grep -q '^SKIPUNZIP=0$' "$MODDIR/customize.sh"
...
echo "Bluetooth Stability Helper tests: PASS"
 "$MODDIR/common/config.sh"
grep -q 'echo "\$\$" > "\$STATE_DIR/service.pid"' "$MODDIR/service.sh"
grep -q 'verify_recovery_outcome()' "$MODDIR/service.sh"
grep -q '^APPLY_APP_OPS_FIXES=0$' "$MODDIR/common/config.sh"
! grep -q '\[ -f "$USERCFG" \] && \. "$USERCFG"' "$MODDIR/service.sh"
! grep -q '\[ -f "$LOCAL_USER_CONFIG" \] && \. "$LOCAL_USER_CONFIG"' "$MODDIR/scripts/diagnostics.sh"
grep -q '^SKIPUNZIP=0$' "$MODDIR/customize.sh"
